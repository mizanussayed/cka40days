...
Check all your existing ingress resources and their classes:
...
There's an official tool to help convert Ingress resources to Gateway API:
...
| Feature | Ingress Annotation | Gateway API Equivalent | Example |
|---------|-------------------|------------------------|---------|
| Basic Routing | `<none>` | HTTPRoute | Route by Domain |
| Path Rewrite | `rewrite-target` | HTTPRoute → URLRewrite filter | URL Rewrite |
| TLS termination | `tls.secretName` | Gateway → Listeners → tls.certificateRefs | TLS termination |
| Redirects | `app-root`, `ssl-redirect` | HTTPRoute → RequestRedirect filter | HTTP redirects |
| Header Mod | `add-header` | HTTPRoute → RequestHeaderModifier filter |  |
| Basic Auth | `auth-type: basic` | Policy Attachment (Targeting HTTPRoute) | BasicAuth |
...
In these examples, we will run `port-forward` to test the old and new endpoints using `curl`. You want to ensure you get similar or same expected outcomes to ensure compatibility on your new endpoint.
...
This is what the [HTTPRoute: Route by Domain](../03-httproute-by-hostname.yaml) looks like.
