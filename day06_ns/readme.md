...
5. Our pod-to-pod connection should work, and you should be able to get a successful response back.
6. Now scale both of our deployments from 1 to 3 replicas.
7. Create two services to expose both of our deployments and name them svc-ns1 and svc-ns2
...
### set namespece to current context
```bash
kubectl config set-context --current -n=ns1
```

## use ng or -n to set namespace in commands
when you want to run a command in a specific namespace, use the -n or --namespace flag:
```bash
kubectl get pods -n ns1
kubectl get pods --namespace=ns2
```

when we work to get all resources in all namespaces, we can use the --all-namespaces or -A flag:
```bash
kubectl get pods --all-namespaces
kubectl get pods -A
```
when we want to delete/get a namespace:
```bash
